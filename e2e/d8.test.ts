...
test.describe('D8 end-to-end interactions', () => {
  ...
  test('renders face0 slot initially and matches visual snapshot', async ({
    ...
    await expect(piece).toHaveScreenshot('d8_e2e-initial.png');
  });

  test('flips die to face7 on keypress and matches visual snapshot', async ({
    page,
  }) => {
    const piece = page.locator('#die');
    await piece.hover();
    await page.keyboard.press('f');
    await expect(piece).toHaveScreenshot('d8_e2e-flipped.png');
  });
  ...
  test('picks up die into hand overlay on hover and keypress', async ({
    ...
    await expect(page).toHaveScreenshot('d8_e2e-picked.png');
  });
});
